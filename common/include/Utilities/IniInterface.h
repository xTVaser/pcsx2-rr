...
public:
    ...
    virtual void Entry(const wxString &var, fs::path &value, const fs::path defvalue = fs::path()) = 0;
...
public:
    ...
    void Entry(const wxString &var, fs::path &value, const fs::path defvalue = fs::path());
...
#define IniEntryDirFile(varname) ini.Entry(wxT(#varname), varname, varname)
