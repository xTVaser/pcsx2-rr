...
void ConsoleLogFrame::OnEnableAllLogging(wxCommandEvent& evt)
{
	...
	// TODO - config - set the value to itself but also true? wat?
	const_cast<Pcsx2Config&>(*g_Conf->emulator).CdvdVerboseReads = g_Conf->emulator->CdvdVerboseReads = true;
...
void ConsoleLogFrame::OnDisableAllLogging(wxCommandEvent& evt)
{
	...
	// Safe to change - it's read only in the core thread and only affects log output.
	const_cast<Pcsx2Config&>(*g_Conf->emulator).CdvdVerboseReads = g_Conf->emulator->CdvdVerboseReads = false;
...
void ConsoleLogFrame::OnLoggingChanged()
{
	...
	if (wxMenuItem* item = GetMenuBar()->FindItem(MenuId_LogSource_CDVD_Info))
		item->Check(g_Conf->emulator->CdvdVerboseReads);
...
#ifndef DISABLE_RECORDING
		GetMenuBar()->Enable(MenuId_LogSource_Start + MenuId_LogSources_Offset_recordingConsole, g_Conf->emulator->EnableRecordingTools);
		GetMenuBar()->Enable(MenuId_LogSource_Start + MenuId_LogSources_Offset_controlInfo, g_Conf->emulator->EnableRecordingTools);
#endif
	}
}
...
void ConsoleLogFrame::OnMoveAround(wxMoveEvent& evt)
{
	...
	if (wxWindow* main = GetParent())
	{
		...
		//Console.WriteLn( "DockCheck: %d", g_Conf->gui->ConLogBox.AutoDock );
		if (m_conf.AutoDock)
		{
...
void ConsoleLogFrame::OnToggleCDVDInfo(wxCommandEvent& evt)
{
	...
	if (evt.GetId() == MenuId_LogSource_CDVD_Info)
	{
		...
		{
			g_Conf->emulator->CdvdVerboseReads = item->IsChecked();
			const_cast<Pcsx2Config&>(*g_Conf->emulator).CdvdVerboseReads = g_Conf->emulator->CdvdVerboseReads; // read-only in core thread, so it's safe to modify.
		}
	}
}
...
{
#if defined(__unix__)
	if ((g_Conf) && (g_Conf->emulator->ConsoleToStdio))
