...
*/
		case McdColS_Size:
			return prefix + (!it.IsPresent ? L"" : (it.IsPSX ? pxsFmt(L"%u MBit", it.SizeInMB) : (it.SizeInMB > 0 ? pxsFmt(L"%u MiB", it.SizeInMB) : L"Auto")));
		case McdColS_Formatted:
			return prefix + (!it.IsPresent ? L"" : (it.IsFormatted ? _("Yes") : _("No")));
		case McdColS_Type:
			return prefix + (!it.IsPresent ? L"" : (it.IsPSX ? _("PSX") : _("PS2")));
		case McdColS_DateModified:
			return prefix + (!it.IsPresent || !it.DateModified.IsValid() ? L"" : it.DateModified.FormatDate());
		case McdColS_DateCreated:
			return prefix + (!it.IsPresent || !it.DateCreated.IsValid() ? L"" : it.DateCreated.FormatDate());

		case McdColS_Filename:
		{
			if (!it.IsPresent && it.Slot != -1)
				return L"";
			...
			if (filepath.SameAs((wxFileName)g_Conf->gui->Folders.MemoryCards))
...
wxListItemAttr* MemoryCardListView_Simple::OnGetItemAttr(long item) const
{
	...
	if (!m_CardProvider)
		return _parent::OnGetItemAttr(item);
	const McdSlotItem& it(m_CardProvider->GetCardForViewIndex(item));
