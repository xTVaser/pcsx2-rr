...
{
	if ((m_MultitapEnabled[0] != g_Conf->emulator->MultitapPort0_Enabled) ||
		(m_MultitapEnabled[1] != g_Conf->emulator->MultitapPort1_Enabled))
	{
		m_MultitapEnabled[0] = g_Conf->emulator->MultitapPort0_Enabled;
		m_MultitapEnabled[1] = g_Conf->emulator->MultitapPort1_Enabled;
...
void Panels::MemoryCardListPanel_Simple::Apply()
{
	...
	{
		g_Conf->gui->Mcd[slot].Type = m_Cards[slot].Type;
		g_Conf->gui->Mcd[slot].Enabled = m_Cards[slot].IsEnabled && m_Cards[slot].IsPresent;
		if (m_Cards[slot].IsPresent)
			g_Conf->gui->Mcd[slot].Filename = m_Cards[slot].Filename;
		else
			g_Conf->gui->Mcd[slot].Filename = "";

		if (g_Conf->gui->Mcd[slot].Enabled)
		{
			used++;
			Console.WriteLn(L"slot[%d]='%s'", slot, WX_STR(g_Conf->gui->Mcd[slot].Filename.GetFullName()));
		}
	}
	if (!used)
...
void Panels::MemoryCardListPanel_Simple::AppStatusEvent_OnSettingsApplied()
{
	for (uint slot = 0; slot < 8; ++slot)
	{
		m_Cards[slot].IsEnabled = g_Conf->gui->Mcd[slot].Enabled;
		m_Cards[slot].Filename = g_Conf->gui->Mcd[slot].Filename;
...
void Panels::MemoryCardListPanel_Simple::DoRefresh()
{
	for (int slot = 0; slot < 8; slot++)
	{
		...
		std::string fullpath((Path::Combine(m_FolderPicker->GetPath(), g_Conf->gui->Mcd[slot].Filename.GetFullName().ToStdString())));
		//std::string fullpath = m_FolderPicker->GetPath() + m_Cards[slot].Filename;

		wxString path(fullpath);

		wxDirName temp(m_FolderPicker->GetPath());
...
void Panels::MemoryCardListPanel_Simple::UiDeleteCard(McdSlotItem& card)
{
	...
	if (card.IsFormatted)
	{
		wxString content;
		content.Printf(
			pxE(L"You are about to delete the formatted memory card '%s'. All data on this card will be lost!  Are you absolutely and quite positively sure?"), card.Filename.GetFullPath());
...
bool Panels::MemoryCardListPanel_Simple::UiDuplicateCard(McdSlotItem& src, McdSlotItem& dest)
{
	...
	if (!((srcfile.FileExists() && wxCopyFile(srcfile.GetFullPath(), destfile.GetFullPath(), true)) || (!srcfile.FileExists() && CopyDirectory(srcfile.GetFullPath(), destfile.GetFullPath()))))
	{
		...
		heading.Printf(pxE(L"Failed: Destination memory card '%s' is in use."),
					   dest.Filename.GetFullPath().ToStdString(), dest.Slot);
	...
	success.Printf(_("Memory card '%s' duplicated to '%s'."),
				   src.Filename.GetFullPath().ToStdString(),
				   dest.Filename.GetFullPath().ToStdString());
...
void Panels::MemoryCardListPanel_Simple::OnOpenItemContextMenu(wxListEvent& evt)
{
	...
	if (idx != wxNOT_FOUND)
	{
		...
		{
			junk->Append(McdMenuId_Create, _("&Create a new"));
...
void Panels::MemoryCardListPanel_Simple::ReadFilesAtMcdFolder()
{
	...
	m_filesystemPlaceholderCard.Filename = "";
...
bool Panels::MemoryCardListPanel_Simple::isFileAssignedAndVisibleOnList(const wxFileName cardFile) const
{
	int i;
	for (i = 0; i < 8; i++)
		if (IsSlotVisible(i) && cardFile == m_Cards[i].Filename)
			return true;
...
bool Panels::MemoryCardListPanel_Simple::isFileAssignedToInternalSlot(const wxFileName cardFile) const
{
	int i;
	for (i = 0; i < 8; i++)
		if (cardFile == m_Cards[i].Filename)
			return true;
