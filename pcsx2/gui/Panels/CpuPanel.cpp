...
Panels::CpuPanelEE::CpuPanelEE(wxWindow* parent)
	: BaseApplicableConfigPanel_SpecificConfig(parent)
{
	*this += Text(pxE(L"Notice: Most games are fine with the default options.")) | StdExpand();
...
{
	Pcsx2Config::CpuOptions& cpuOps(g_Conf->emulator->Cpu);
...
void Panels::AdvancedOptionsFPU::ApplyConfigToGui(GuiConfig& configToApply, int flags)
{
	...
	if (recOps.fpuFullMode)
		m_ClampModePanel->SetSelection(3);
	else if (recOps.fpuExtraOverflow)
		m_ClampModePanel->SetSelection(2);
	else if (recOps.fpuOverflow)
		m_ClampModePanel->SetSelection(1);
	else
...
void Panels::AdvancedOptionsVU::ApplyConfigToGui(GuiConfig& configToApply, int flags)
{
	...
	if (recOps.vuSignOverflow)
		m_ClampModePanel->SetSelection(3);
	else if (recOps.vuExtraOverflow)
		m_ClampModePanel->SetSelection(2);
	else if (recOps.vuOverflow)
		m_ClampModePanel->SetSelection(1);
	else
		m_ClampModePanel->SetSelection(0);
