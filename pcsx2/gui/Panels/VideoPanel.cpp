...
{
	FramerateOptions& appfps(g_Conf->gui->Framerate);
	Pcsx2Config::GSOptions& gsconf(g_Conf->emulator->GS);
	...
	: BaseApplicableConfigPanel_SpecificConfig(parent)
{
	...
	*this += Text(pxE(L"Notice: Due to PS2 hardware design, precise frame skipping is impossible. Enabling it will cause severe graphical errors in some games.")) | StdExpand();
...
{
	g_Conf->emulator->GS.SynchronousMTGS = m_check_SynchronousGS->GetValue();
}
