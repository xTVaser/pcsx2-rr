...
void ApplyOverValidStateEvent::InvokeEvent()
{
	...
	if (result == wxID_CANCEL)
		throw Exception::CannotApplySettings(m_owner->GetApplicableConfigPanel()).Quiet().SetDiagMsg(L"Cannot apply settings: canceled by user because plugins changed while the emulation state was active.");
}
...
void SysExecEvent_ApplyPlugins::InvokeEvent()
{
	...
	if (buffer)
		CoreThread.UploadStateCopy(*buffer);
		...
		if (sel != wxNOT_FOUND)
		{
			m_ComponentBoxes->GetConfigButton(pi->id).Enable(
				(m_FileList == NULL || m_FileList->size() == 0) ? false :
																  g_Conf->gui->FullpathMatchTest(pi->id, (*m_FileList)[((uptr)box.GetClientData(sel))]));
		}
	});
}
...
{
	return pxsFmt(pxE(L"All plugins must have valid selections for %s to run.  If you are unable to make a valid selection due to missing plugins or an incomplete install of %s, then press Cancel to close the Configuration panel."), WX_STR(pxGetAppName()), WX_STR(pxGetAppName()));
}
...
void Panels::PluginSelectorPanel::Apply()
{
	...
	ForPlugins([&](const PluginInfo* pi) {
		...
		g_Conf->gui->BaseFilenames.Plugins[pid] = GetFilename((uptr)m_ComponentBoxes->Get(pid).GetClientData(sel));
	});
	...
	do
	{
		if (g_Conf->gui->FullpathTo(pi->id) != g_Conf->gui->FullpathTo(pi->id))
			break;
	} while (++pi, pi->shortname != NULL);

	if (pi->shortname == NULL)
		return; // no plugins changed? nothing left to do!
...
bool Panels::PluginSelectorPanel::ValidateEnumerationStatus()
{
	if (m_EnumeratorThread)
		return true; // Cant reset file lists while we're busy enumerating...
...
{
	if (IsBeingDeleted() || m_ComponentBoxes->IsBeingDeleted())
		return;
	...
	IfPlugins([&](const PluginInfo* pi) {
		...
		if (box.GetId() == evt.GetId())
		{
			...
			bool isSame = (!CorePlugins.AreLoaded()) || g_Conf->gui->FullpathMatchTest(pi->id, (*m_FileList)[(uptr)box.GetClientData(box.GetSelection())]);
			m_ComponentBoxes->GetConfigButton(pi->id).Enable(isSame);

			if (!isSame)
				evt.Skip(); // enabled Apply button! :D
			return true;
		}
		return false;
	});
...
void Panels::PluginSelectorPanel::OnConfigure_Clicked(wxCommandEvent& evt)
{
	...
	if (CorePlugins.AreLoaded() && !g_Conf->gui->FullpathMatchTest(pid, filename))
	{
		...
		else if (m_ComponentBoxes->Get(pid).GetSelection() == wxNOT_FOUND)
		{
			if (pid == PluginId_GS)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					if (x86caps.hasAVX2 && str.Contains("AVX2"))
						index_avx2 = i;
					if (x86caps.hasStreamingSIMD4Extensions && str.Contains("SSE4"))
						index_sse4 = i;
				...
				if (index_avx2 >= 0)
					m_ComponentBoxes->Get(pid).SetSelection(index_avx2);
				else if (index_sse4 >= 0)
					m_ComponentBoxes->Get(pid).SetSelection(index_sse4);
				else if (index_sse2 >= 0)
					m_ComponentBoxes->Get(pid).SetSelection(index_sse2);
				else
...
void Panels::PluginSelectorPanel::OnProgress(wxCommandEvent& evt)
{
	...
	if (m_EnumeratorThread.get() != (EnumThread*)evt.GetClientData())
		return;
	...
	ForPlugins([&](const PluginInfo* pi) {
		...
		if (result.TypeMask & pi->typemask)
		{
			if (result.PassedTest & pi->typemask)
			{
				...
				if (g_Conf->gui->FullpathMatchTest(pid, (*m_FileList)[evtidx]))
				{
