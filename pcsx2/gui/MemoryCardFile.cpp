...
wxFileName FileMcd_GetSimpleName(uint slot)
{
	if( FileMcd_IsMultitapSlot(slot) )
		return g_Conf->gui->Folders.MemoryCards + wxsFormat( L"Mcd-Multitap%u-Slot%02u.ps2", FileMcd_GetMtapPort(slot)+1, FileMcd_GetMtapSlot(slot)+1 );
	else
		return g_Conf->gui->Folders.MemoryCards + wxsFormat( L"Mcd%03u.ps2", slot+1 );
}
*/
...
void FileMemoryCard::Open()
{
	for (int slot = 0; slot < 8; ++slot)
	{
		...
		wxFileName fname(g_Conf->gui->FullpathToMcd(slot));
		...
		if (g_Conf->gui->Mcd[slot].Type != MemoryCardType::MemoryCard_File)
		{
...
static void PS2E_CALLBACK FileMcd_EmuOpen(PS2E_THISPTR thisptr, const PS2E_SessionInfo* session)
{
	...
	for (uint slot = 0; slot < 8; ++slot)
	{
		if (g_Conf->gui->Mcd[slot].Enabled)
		{
			...
			g_Conf->gui->Mcd[slot].Type = type;
		}
	}

	thisptr->impl.Open();
	thisptr->implFolder.SetFiltering(g_Conf->emulator->McdFolderAutoManage);
...
static s32 PS2E_CALLBACK FileMcd_IsPresent(PS2E_THISPTR thisptr, uint port, uint slot)
{
	...
	switch (g_Conf->gui->Mcd[combinedSlot].Type)
	{
...
static bool PS2E_CALLBACK FileMcd_ReIndex(PS2E_THISPTR thisptr, uint port, uint slot, const wxString& filter)
{
	...
	switch (g_Conf->gui->Mcd[combinedSlot].Type)
	{
		...
		//	break;
		case MemoryCardType::MemoryCard_Folder:
			return thisptr->implFolder.ReIndex(combinedSlot, g_Conf->emulator->McdFolderAutoManage, filter);
			break;
		default:
			return false;
	}
