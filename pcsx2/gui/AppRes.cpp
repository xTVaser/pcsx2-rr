...
RecentIsoList::RecentIsoList(int firstIdForMenuItems_or_wxID_ANY)
{
	...
	Menu->Append(MenuId_IsoBrowse, _("Browse..."), _("Browse for an ISO that is not in your recent history."))->Enable(!g_Conf->gui->AskOnBoot);
	Menu->Check(MenuId_Ask_On_Booting, g_Conf->gui->AskOnBoot);
...
wxImageList& Pcsx2App::GetImgList_Config()
{
	...
	if (!images)
	{
		int image_size = MSW_GetDPIScale() * g_Conf->gui->Listbook_ImageSize;
...
wxImageList& Pcsx2App::GetImgList_Toolbars()
{
	...
	if (!images)
	{
		const int imgSize = g_Conf->gui->Toolbar_ImageSize ? 64 : 32;
