	...
	, cpu(_cpu)
{
	...
	setRowSize(g_Conf->emulator->Debugger.MemoryViewBytesPerRow);
	...
	menu.Check(ID_MEMVIEW_ALIGNWINDOW, g_Conf->emulator->Debugger.AlignMemoryWindowStart);
...
void CtrlMemView::onPopupClick(wxCommandEvent& evt)
{
	...
	switch (evt.GetId())
	{
		...
		case ID_MEMVIEW_ALIGNWINDOW:
			g_Conf->emulator->Debugger.AlignMemoryWindowStart = evt.IsChecked();
			if (g_Conf->emulator->Debugger.AlignMemoryWindowStart)
			{
