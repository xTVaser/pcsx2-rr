	...
	wxFrame(parent, id, title, pos, size, wxDEFAULT_FRAME_STYLE)
{
	...
	VirtualPad::m_controllerPort = controllerPort;
	...
	// NOTE: Order MATTERS, these match enum defined in PadData.h
	wxSlider* tempAnalogSliders[4] = { m_leftAnalogXVal, m_leftAnalogYVal, m_rightAnalogXVal, m_rightAnalogYVal };
	std::copy(std::begin(tempAnalogSliders), std::end(tempAnalogSliders), std::begin(m_analogSliders));

	// NOTE: Order MATTERS, these match enum defined in PadData.h
	wxSpinCtrl* tempAnalogVals[4] = { m_leftAnalogXValPrecise, m_leftAnalogYValPrecise, m_rightAnalogXValPrecise, m_rightAnalogYValPrecise };
	std::copy(std::begin(tempAnalogVals), std::end(tempAnalogVals), std::begin(m_analogVals));
...
void VirtualPad::onButtonPress(wxCommandEvent & event)
{
	...
	if (buttonId != -1)
	{
		...
		if (event.IsChecked())
		{
			if (buttonId < 12)
			{
				pressure = m_buttonsPressure[buttonId]->GetValue();
			}
			else
			{
				pressure = 255;
		...
		g_RecordingInput.setButtonState(m_controllerPort, PadData_NormalButton(buttonId), pressure);
	}
}
...
void VirtualPad::onPressureChange(wxSpinEvent & event)
{
	...
	if (spinnerId != -1)
	{
		...
		if (event.IsChecked())
		{
			pressure = m_buttonsPressure[spinnerId]->GetValue();
		}
		g_RecordingInput.setButtonState(m_controllerPort, PadData_NormalButton(spinnerId), pressure);
	}
}
...
void VirtualPad::onAnalogSliderChange(wxCommandEvent & event)
{
	...
	if (sliderId != -1)
	{
		if (sliderId % 2 == 0)
		{
			m_analogVals[sliderId]->SetValue(event.GetInt());
		}
		else
		{
			m_analogVals[sliderId]->SetValue(event.GetInt() * -1);
		}

		g_RecordingInput.updateAnalog(m_controllerPort, PadData_AnalogVector(sliderId), event.GetInt() + 127);
	}
}
...
void VirtualPad::onAnalogValChange(wxSpinEvent & event)
{
	...
	if (spinnerId != -1)
	{
		m_analogVals[spinnerId]->SetValue(event.GetInt());
		g_RecordingInput.updateAnalog(m_controllerPort, PadData_AnalogVector(spinnerId), event.GetInt() + 127);
	}
}
