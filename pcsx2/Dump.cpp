...
void iDumpPsxRegisters(u32 startpc, u32 temp)
{
	...
	for (i = 0; i < 34; i += 2)
		__Log("%spsx%s: %x %x", pstr, disRNameGPR[i], psxRegs.GPR.r[i], psxRegs.GPR.r[i + 1]);
...
void iDumpRegisters(u32 startpc, u32 temp)
{
	...
	for (i = 1; i < 32; ++i)
		__Log("%s: %x_%x_%x_%x", disRNameGPR[i], cpuRegs.GPR.r[i].UL[3], cpuRegs.GPR.r[i].UL[2], cpuRegs.GPR.r[i].UL[1], cpuRegs.GPR.r[i].UL[0]);
	...
	for (i = 0; i < 32; ++i)
		__Log("%sf%d: %x %x", pstr, i, fpuRegs.fpr[i].UL, fpuRegs.fprc[i]);
	for (i = 1; i < 32; ++i)
		__Log("%svf%d: %x %x %x %x, vi: %x", pstr, i, VU0.VF[i].UL[3], VU0.VF[i].UL[2], VU0.VF[i].UL[1], VU0.VF[i].UL[0], VU0.VI[i].UL);
...
void iDumpBlock(u32 ee_pc, u32 ee_size, uptr x86_pc, u32 x86_size)
{
	...
	g_Conf->gui->Folders.Logs;
	wxString dump_filename = Path::Combine(g_Conf->gui->Folders.Logs, (std::string)wxsFormat(L"R5900dump_%.8X:%.8X.txt", ee_pc, ee_end));
...
#ifdef __linux__
	wxString obj_filename = Path::Combine(g_Conf->gui->Folders.Logs, "objdump_tmp.o");
...
void iDumpBlock(int startpc, u8* ptr)
{
	...
	g_Conf->gui->Folders.Logs;
	AsciiFile eff(
		Path::Combine(g_Conf->gui->Folders.Logs, (std::string)wxsFormat(L"R5900dump%.8X.txt", startpc)), L"w");
