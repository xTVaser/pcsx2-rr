...
void iDumpBlock(u32 ee_pc, u32 ee_size, uptr x86_pc, u32 x86_size)
{
	...
	g_Conf->Folders.Logs;
	wxString dump_filename = Path::Combine( g_Conf->Folders.Logs, (std::string)wxsFormat(L"R5900dump_%.8X:%.8X.txt", ee_pc, ee_end) );
...
void iDumpBlock( int startpc, u8 * ptr )
{
	...
	g_Conf->Folders.Logs;
	AsciiFile eff(
		Path::Combine( g_Conf->Folders.Logs, (std::string)wxsFormat(L"R5900dump%.8X.txt", startpc) ), L"w"
	);
