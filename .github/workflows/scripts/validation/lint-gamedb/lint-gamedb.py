...
def validate_game_option(serial, key, value): # noqa: C901
    ...
    if key == "roundModes" and isinstance(value, dict):
        for roundMode, roundValue in value.items():
    ...
    if key == "clampModes" and isinstance(value, dict):
        for clampMode, clampValue in value.items():
    ...
    if key == "speedHacks" and isinstance(value, dict):
        for speedhack, speedhackValue in value.items():
...
with open(file_path) as f:
    ...
    for serial, game_options in gamedb.items():
        ...
        for key, value in game_options.items():
            validate_game_option(serial, key, value)
